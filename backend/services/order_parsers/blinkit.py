...
class BlinkitParser:
    ...
    def matches(sender: str, subject: str) -> bool:
        return "blinkit" in sender.lower() or "grofers" in sender.lower() or "blinkit.com" in sender.lower()
    ...
    def parse(msg_id: str, sender: str, subject: str, snippet: str, body_text: str, body_html: str, date_str: Optional[str]) -> Optional[Dict[str, Any]]:
        # Blinkit Order ID: BLNK123456 or numeric order ID
        order_match = re.search(r"\b((?:BLNK|ORD)?\d{6,14})\b", f"{subject} {snippet} {body_text}", re.IGNORECASE)
        order_number = order_match.group(1) if order_match else None
        
        lifecycle = detect_lifecycle_status(subject, snippet, body_text)
        
        product_name = None
        subj_match = re.search(r"(?:order of|for)\s+(.+?)(?:\s+has|\s+is|\s+delivered|\s+received|\.|$)", subject, re.IGNORECASE)
        if subj_match:
        ...
        if not product_name:
            ...
            for line in lines[:10]:
                if any(kw in line.lower() for kw in ["blinkit", "grofers", "order id", "bill amount", "delivery partner", "10 minutes"]):
                    continue
                if len(line) > 6:
        ...
        if not product_name:
            product_name = "Blinkit Quick Delivery Order"
        ...
        if body_html:
            try:
                ...
                for img in soup.find_all("img"):
                    src = img.get("src", "")
                    if "blinkit" in src or "grofers" in src:
                        image_url = src
                        break
            except Exception:
                pass
