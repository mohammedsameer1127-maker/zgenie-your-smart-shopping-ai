...
class FlipkartParser:
    ...
    def matches(sender: str, subject: str) -> bool:
        return "flipkart" in sender.lower() or "flipkart.com" in sender.lower()
    ...
    def parse(msg_id: str, sender: str, subject: str, snippet: str, body_text: str, body_html: str, date_str: Optional[str]) -> Optional[Dict[str, Any]]:
        # Flipkart Order Number: OD123456789012345600
        order_match = re.search(r"\b(OD\d{15,22})\b", f"{subject} {snippet} {body_text}", re.IGNORECASE)
        order_number = order_match.group(1).upper() if order_match else None
        
        lifecycle = detect_lifecycle_status(subject, snippet, body_text)
        
        product_name = None
        subj_match = re.search(r"for\s+(.+?)(?:\s+is|\s+has|\s+placed|\s+confirmed|\s+delivered|\s+shipped|\.|$)", subject, re.IGNORECASE)
        if subj_match:
        ...
        if not product_name:
            ...
            for line in lines[:10]:
                if any(kw in line.lower() for kw in ["flipkart", "order id", "track order", "download invoice", "view details"]):
                    continue
                if len(line) > 8:
        ...
        if not product_name:
            product_name = "Flipkart Purchase Item"
        ...
        if body_html:
            try:
                ...
                for img in soup.find_all("img"):
                    src = img.get("src", "")
                    if "rukminim" in src or "fkcdn" in src:
                        image_url = src
                        break
            except Exception:
                pass
