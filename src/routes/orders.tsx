...
function OrdersPage() {
  ...
  useEffect(() => {
    if (searchParams.gmail_status === "connected") {
      toast.success("Gmail connected successfully! You can now sync your shopping orders.");
      fetchStatusAndOrders();
    } else if (searchParams.gmail_status === "error") {
      toast.error(`Gmail connection failed: ${searchParams.error || "Please try again."}`);
    }
  }, [searchParams.gmail_status, searchParams.error]);
                  ...
                  <div className="flex items-start justify-between gap-2 border-b border-border/60 pb-3">
                    ...
                    <div className="text-right">
                      <span className="text-[11px] text-muted-foreground flex items-center gap-1 font-medium">
                        ...
                        {formatTimestamp(order.order_date, order.order_time)}
                      </span>
                    </div>
                  </div>
