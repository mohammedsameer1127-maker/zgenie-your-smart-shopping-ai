...
import { Eye, EyeOff, Mail, Lock, User, Sparkles, ArrowRight, ShieldCheck, CheckCircle2 } from "lucide-react";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "ZGenie — Sign In / Sign Up" },
      {
        ...
        content:
          "The ultimate AI-powered smart shopping and product comparison engine. Sign in or register today.",
      },
    ],
  }),
  component: AuthPage,
...
function AuthPage() {
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 grid grid-cols-2 rounded-2xl border border-border/80 bg-muted/50 p-1.5 backdrop-blur-sm">
        <button
          type="button"
          onClick={() => setTab("signin")}
          className={`flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold transition-all duration-200 ${
            tab === "signin"
  ...
  return (
    <form className="space-y-4" onSubmit={handleSignIn}>
      <div className="space-y-2">
        <Label htmlFor="si-email" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          Email Address
        </Label>
        <div className="relative">
          ...
          <Input
            ...
            className="h-11 rounded-xl pl-10 bg-background/80"
            required
          />
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <Label htmlFor="si-password" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Password
          </Label>
          <a
            href="#"
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder="••••••••••••"
            className="h-11 rounded-xl px-10 bg-background/80"
            required
          />
          <button
            type="button"
      ...
      <div className="flex items-center gap-2 pt-1">
        <Checkbox id="si-remember" defaultChecked />
        <Label htmlFor="si-remember" className="text-xs font-medium text-muted-foreground cursor-pointer">
      ...
      <Button
        type="submit"
        disabled={loading}
        className="h-11 w-full rounded-xl text-sm font-bold shadow-md transition-transform active:scale-[0.99] gap-2"
      >
        {loading ? (
          "Signing in..."
        ) : (
...
function SignUpForm({ onSwitchToSignIn }: { onSwitchToSignIn: () => void }) {
  ...
  return (
    <form className="space-y-4" onSubmit={handleSignUp}>
      <div className="space-y-2">
        <Label htmlFor="su-name" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          Full Name
        </Label>
        <div className="relative">
      ...
      <div className="space-y-2">
        <Label htmlFor="su-email" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          Email Address
        </Label>
        <div className="relative">
          <Mail className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="su-password" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Password
          </Label>
          <div className="relative">
            <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        ...
        <div className="space-y-2">
          <Label htmlFor="su-confirm" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Confirm Password
          </Label>
          <div className="relative">
            <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
      ...
      <div className="flex items-start gap-2 pt-1">
        <Checkbox
          ...
          required
        />
        <Label htmlFor="su-terms" className="text-xs font-normal leading-relaxed text-muted-foreground cursor-pointer">
          I agree to the{" "}
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <Button
        variant="outline"
        type="button"
        onClick={() => toast.info("Google Sign-In integration ready!")}
        className="h-11 rounded-xl gap-2 hover:bg-muted/80 font-semibold text-xs"
      >
        <GoogleIcon /> Google
      </Button>
      <Button
        variant="outline"
        type="button"
        onClick={() => toast.info("Apple Sign-In integration ready!")}
        className="h-11 rounded-xl gap-2 hover:bg-muted/80 font-semibold text-xs"
      >
        <AppleIcon /> Apple
      </Button>
